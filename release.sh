...
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  xdelta3-gui Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
validate_version() {
    ...
    if ! [[ $clean_version =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
        ...
        echo "Expected formats: 1.0.0, v1.0.0, or YY.MM (like 25.4)"
        exit 1
    fi
...
update_aur_package() {
    ...
    local tarball_url="https://github.com/AdrianTM/xdelta3-gui/archive/refs/tags/${version}.tar.gz"
